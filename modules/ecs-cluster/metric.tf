#####################################
# Amazon ASG Service Metric Scaling #
#####################################

resource "aws_cloudwatch_metric_alarm" "firing" {
  alarm_name          = "${var.name_prefix}-${var.cluster_name}-firing"
  ...
  alarm_actions     = concat([aws_autoscaling_policy.scale_out.arn], var.additional_alarm_actions)
  tags              = var.tags
}

resource "aws_cloudwatch_metric_alarm" "relax" {
  alarm_name          = "${var.name_prefix}-${var.cluster_name}-relax"
  ...
  alarm_actions     = concat([aws_autoscaling_policy.scale_in.arn], var.additional_alarm_actions)
  tags              = var.tags
}
